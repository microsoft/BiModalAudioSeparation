...
class CLAP_Module(torch.nn.Module):
    ...
    def my_get_audio_embedding_from_filelist(self, x, use_tensor=False):
        """get audio embeddings from the audio file list
        ...
        for f in x:
            # load the waveform of the shape (T,), should resample to 48000
            audio_raw, rate = librosa.load(f, sr=None, mono=True)
            center = len(audio_raw) // 2 + 1

            n = rate * 10

            len_raw = len(audio_raw)

            if len_raw < n:
                rf = int(n / len_raw)
                audio_raw = np.tile(audio_raw, rf)

            start = max(0, center - (n // 2))
            end = min(len_raw, center + (n // 2))

            audio = np.zeros(n)

            audio[n//2-(center-start): n//2+(end-center)] = \
                audio_raw[start:end]

            audio_waveform = librosa.resample(audio, orig_sr=rate, target_sr=48000)
        ...
        if len(audio_input) > 128:
            audio_embed = []

            for i in range(0, len(audio_input), 128):
                end = min(len(audio_input), i + 128)

                audio_embed.append(self.model.get_audio_embedding(audio_input[i : end]))
