...
class CLAP(nn.Module):
    ...
    def encode_text(self, text, device, return_hidden_states=False):
        if self.text_branch_type == "transformer":
            ...
            hidden_states, x = output['last_hidden_state'], output['pooler_output']
    ...
    def get_text_embedding(self, data, return_hidden_states=False):
        ...
        if return_hidden_states:
            text_embeds, hidden_states = self.encode_text(data, device=device, return_hidden_states=return_hidden_states)
            text_embeds = F.normalize(text_embeds, dim=-1)

            return text_embeds, hidden_states
