...
class MusicDataset(torchdata.Dataset):
    def __init__(self, list_sample, num_mix, opt, split):
        ...
        if self.split == "test":
            solo_list = "data/music/annotations/test.csv"
            self.test_files = []
            for row in csv.reader(open(solo_list, 'r'), delimiter=','):
                if len(row) < 2:
                    continue
                self.test_files.append(row)
        ...
        if isinstance(list_sample, str):
            ...
            if self.split == "train":
                self.list_sample = self.list_sample * 50
    ...
    def _load_audio(self, path, center_timestamp, nearest_resample=False):
        ...
        if self.split == "train":
            center = random.randint(1, center)
    ...
    def __getitem__(self, index):
        ...
        for n, infoN in enumerate(infos):
            #print(infoN)
            path_audioN, _, duration = infoN

            if self.split == 'train':
                # random, not to sample start and end n-frames
                center_time = random.randint(
                    1, int(duration))
            else:
                center_time = int(duration) // 2 + 1

            center_times[n] = center_time

            path_audios[n] = os.path.join("data/music/audio", path_audioN[1:])
